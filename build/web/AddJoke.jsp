<%-- 
    Document   : AddJoke
    Created on : 22 May 2023, 14:06:48
    Author     : jarro
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<%@page import="Models.*"%>
<%@page import="java.util.List"%>
<!DOCTYPE html>
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <link rel="stylesheet" href="styles.css">
        <title>Add a Joke</title>
    </head>
    <body>
        <h1>Enter Your Joke's Details Below:</h1>
        <%
            String message = (String) request.getAttribute("message");
            if(message==null) {
                message = "";
            }
            List<Category> categories = (List<Category>) request.getSession(false).getAttribute("categories");
        %>
        <h1><%=message%></h1>
        <form action="ClientController" method="get">
            <p><label for="JokeDescription">Joke Description:</label></p> 
            <textarea class="inputTextArea" name="JokeDescription" rows="4" cols="50" required></textarea>
            <br>
            <p style="text-align: center;"><label class="jokeCategories"  for="categories">Joke Categories:</label></p>
            <br>
            <div class="scroll">
            <%
                int i = 0;
                for(Category category:categories) {
            %>
            <label class="container" for="<%=category.getCategoryID()%>">
                <%=category.getName()%>
                <input type="checkbox" id="<%=category.getCategoryID()%>" name="<%=category.getCategoryID()%>" value="<%=category.getCategoryID()%>"/>
                <span class="checkmark"></span>
            </label>
            <br>
            <%      i++;
                }%>
            </div>
            <p style="text-align: center;"><input type="submit" name="submit" value="AddJoke"></p>
        </form>
            <br>
        <p style="text-align: center;"><a href="ClientController?submit=menu">Back to Menu</a></p>
    </body>
</html>
